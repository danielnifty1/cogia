...
$getevent="SELECT* FROM photos where eventid ='$selectedevent' and status=1";
$checkrst=mysqli_query($conn,$getevent);
$checkrtn=mysqli_num_rows($checkrst);

if($checkrtn){
	while ($row=mysqli_fetch_assoc($checkrst)) {
		$gallaryname=$row['gallaryname'];
	}
}
...
<section id="main-content">
 	...
 	$query = "SELECT* from photo_gallary where status=0 and eventid='$selectedevent'";
	...
	<section class="wrapper">
		<div class="row">
			<select id="cat" class="form-contreol" style="bacdkground: black;">
...
<?php 
 ...
 if($checkrtn){ ?>
		...
		<div class="row"  >
			...
			<h4 style="text-align: center;">Photos of <?php echo base64_decode($gallaryname) ?></h4>		
				...
				<table class="table" >
					<tr>
						...
						<th>Delete</th>	
							...
							<?php 
		       								//=$_SESSION['adminname'];
									$query = "SELECT* from photo_gallary where status=0 and eventid='$selectedevent' order by id desc LIMIT ".$page_first_result.','.$result_per_page;
										$result = mysqli_query($conn, $query);//runs the connection to dsatabase and then run the query
		...
		<div class="row">
				<div class="col-md-12 col-md-push-0 ">
					...
					for ($page=1; $page <=$number_of_pages ; $page++) { 
						echo ' <a class="btn-sm btn-info btn" href="photos?event='.$selectedevent.'&page='.$page.'">'.$page.'</a> ';
					} ?>
					</center>
				</div>
		</div>
